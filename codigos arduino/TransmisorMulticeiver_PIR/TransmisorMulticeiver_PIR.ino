/*  ----------------------------------------------------------------
    ...
    Codigo del PTX2 (Alarma con sensor PIR)
  --------------------------------------------------------------------
*/
...
direccion2 = 0xB3B4B5B6F1LL (Este nodo) 
...
int Alarm_out =5; //Pin de salida de la alarma
int Alarm_modo =4; //Pin indicador de alarma Armada o Desarmada
...
//  radio.setRetries(15, 15); //Configuracion del numero de intentos
  ...
  radio.openReadingPipe(0, direccion2); 
...
void loop()
{
      ...
      if (radio.available())      // Comprobacion de datos recibidos
      {
        ...
        //Acciones
        if (op == 'b') //Apagar alarma, pero siempre esta armada
        {
          Alarma = false;
          digitalWrite(Alarm_out,LOW);
        } else if (op == 'O') //Armar la alarma, modo On
        {
          ...
          interrupts();
          delay(300);//Espera aque se den las llamadas a interrupcion pendientes
...
void Alarma_On() //Alarma disparada
  {
    ...
    char fun = 'B';
    radio.openWritingPipe(direccion2); //Se abre el puerto de escritura
...
void Alarma_Off() //Boton de apagado de alarma disparado
  {               
    ...
    char fun = 'E'; //Letra para informar que la alarma se desactivo manualmente
    radio.openWritingPipe(direccion2); //Se abre el puerto de escritura
    radio.write(&fun, 1); //Se envia la notificacion de que la alarma fue activada.
    delay(200);
    }
