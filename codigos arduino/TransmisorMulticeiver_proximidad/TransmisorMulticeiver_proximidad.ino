...
*/
const uint64_t direccion3 = 0xB3B4B5B6CDLL; // En esta parte LL significa LongLong para asegurar el dato de 64bits
...
void loop()
{
      ...
      if (radio.available())      // Comprobacion de datos recibidos
      {
        ...
        //Acciones
        if (op == 'c') //Apagar alarma, pero siempre esta armada
        {
          Alarma = false;
          digitalWrite(Alarm_out,LOW);
        } else if (op == 'O') //Armar la alarma, modo On
        {
          ...
          interrupts();
          delay(250);//Espera aque se den las llamadas a interrupcion pendientes
          armado=1; //Reactivar las acciones de las interrupciones
          Alarma = false;
          modo=1; //Poner modo en Alarma Armada
          digitalWrite(Alarm_out,LOW);
